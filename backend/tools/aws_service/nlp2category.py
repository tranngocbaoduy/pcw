...
class NLPCategory2Code:
    ...
    def get_index_word(self, word):
        ...
        """
        return (
            self.model.get_feature_names().index(word) if self.model != None else None
        )
    ...
    def get_code_by(self, corpus):
        ...
        for sentence in corpus:
            ...
            if len(filtered_longest_sentence) == 0:
                ...
                SENT_2_CODE_LONGEST_WORD.append(
                    self.get_longest_sentence(filtered_longest_sentence)
                )
    ...
    def build(self):
        ...
        (
            self.CODE_FROM_INIT_SENTENCE,
            self.CODE_LONGEST_FROM_INIT_SENTENCE,
        ) = self.get_code_by(self.init_corpus)
        (
            self.CODE_FROM_FILTER_SENTENCE,
            self.CODE_LONGEST_FROM_FILTER_SENTENCE,
        ) = self.get_code_by(self.filter_corpus())
        self.CODE_FROM_URL, self.CODE_LONGEST_FROM_URL = self.get_code_by(
            self.corpus_url
        )
    ...
    def get_code_longest_of_sentence(self, sent):
        ...
        return {
            ...
            "by_url": self.CODE_LONGEST_FROM_URL[index]
            .replace(" ", "")
            .replace("com/", ""),
        }
