...
def pre_handle_goods_item(raw_goods_item, page_item):
    ...
    item = {"PK": PK, "SK": SK, "UPDATE_AT": update_at, **goods_item}
    return item
...
def extract_goods_from_page_item(
    page_item,
    is_pattern_xpath=False,
    is_save_html=False,
):
    ...
    if is_pattern_xpath:
        ...
        with open(
            os.path.join(
                file_dir,
                "model/common",
                "xpath_{}-{}.json".format(encode_url, decode_url),
            ),
            "r",
            encoding="utf-8",
        ) as file:
    ...
    if "IS_FROM_API" in page_item.keys() and page_item["IS_FROM_API"]:
        try:
            ...
            new_goods_item["IS_CONFIDENT"] = (
                True if (total_attr - count_invalid) / total_attr > 0.5 else False
            )
            ...
            if name == "price" or name == "list_price":
                ...
                if src_url == "" or len(src_url) == 0:
                    ...
                    for item in data_srcs:
                        ...
                        if u == None or len(u) == 0 or "javascript:void(0)" in u:
                            u = item.attrib.get("data-thumb")
                        if u == None or len(u) == 0 or "javascript:void(0)" in u:
                            u = item.attrib.get("href")
                        if u != None:
                            src_url.append(
                                "https://" + urlsafre_decode(page_item["PK"]) + u
                            )

                goods_item[name] = [
                    item for item in src_url if item and item.startswith("https:")
                ]
                goods_item["THUMBNAIL_URL"] = (
                    [goods_item[name][0]] if len(goods_item[name]) != 0 else []
                )
        ...
        new_goods_item["PRICE"] = (
            new_goods_item["PRICE"].replace("đ", "").replace(".", "").replace(",", "")
        )
        new_goods_item["LIST_PRICE"] = (
            new_goods_item["LIST_PRICE"]
            .replace("đ", "")
            .replace(".", "")
            .replace(",", "")
        )
        new_goods_item["DISCOUNT_RATE"] = (
            new_goods_item["DISCOUNT_RATE"].replace("%", "").replace(",", "")
        )
        if new_goods_item["PRICE"] and new_goods_item["LIST_PRICE"]:
            ...
            if list_price < price:
                list_price = price
            if new_goods_item["DISCOUNT_RATE"] == "TYPE1":
                new_goods_item["DISCOUNT_RATE"] = -1 * int(
        ...
        for page_item in tqdm(page_items[:]):
            ...
            if new_goods_item is not None:
                ...
                if new_goods_item["PRICE"] == "":
                    ...
                    if (
                        new_goods_item["PRICE"] == ""
                        and new_goods_item["LIST_PRICE"] == ""
                        and new_goods_item["DISCOUNT_RATE"] == "TYPE1"
                    ):
                        print("SKIP")
                        count_skip_item += 1
                        continue
                    ...
                    if (
                        "IMAGES" not in new_goods_item.keys()
                        or len(new_goods_item["IMAGES"]) == 0
                    ):
                        print("no Image count_upload_file", count_upload_file)
                    if new_goods_item["IS_CONFIDENT"]:
                        for k, v in new_goods_item.items():
                            if k not in stat_attrib:
                                if type(v) == str and len(v) > 250:
                                    stat_attrib[k] = 1
                            else:
                                if type(v) == str and len(v) > 250:
                                    stat_attrib[k] += 1
...
def save_file_local(new_goods_item, page_item):
    ...
    with open(os.path.join(func_dir_data, "index.json"), "w") as file:
        ...
        param = {"raw": page_item, "processed": new_goods_item}
...
def put_goods_item_to_db(item):
    ...
    try:
        if item:
            goods_table.put_item(Item=item)
    except:
        print("delete")
        goods_table.delete_item(Key={"PK": item["PK"], "SK": item["SK"]})
...
def delete_page_item_db(item):
    ...
    """
    try:
        pages_table.delete_item(Key={"PK": item["PK"], "SK": item["SK"]})
    except:
        return False
    return True
...
def update_batch_page_item(page_item):
    with pages_table.batch_writer() as batch:
        batch.put_item(
            Item={**page_item, "PK": page_item["PK"].replace("UNPROCESSED", "FINISHED")}
        )
        batch.delete_item(Key={"PK": page_item["PK"], "SK": page_item["SK"]})
...
def update_backdata_batch_page_item():
    scan_kwargs = {
        ...
        "ExpressionAttributeValues": {":pk": "FINISHED"},
    }
...
def get_page_item(PK, SK):
    return pages_table.get_item(
        TableName="pcw-dev-PAGES", Key={"PK": PK, "SK": SK}
    ).get("Item", False)
