...
def build_item():
    ...
    for id_cate, items in helper.NEW_CATEGORY_ITEM_SET.items():
        for item in items:
            if (
                ...
                or item["CODE_PRODUCT"] == ""
                or "CODE_PRODUCT" not in item.keys()
            ):
                count_err_item += 1
            else:
                if (
                    "{}@{}".format(id_cate, item["CODE_PRODUCT"])
                    not in stat_code.keys()
                ):
                    stat_code["{}@{}".format(id_cate, item["CODE_PRODUCT"])] = 1
                    stat_SK["{}@{}".format(id_cate, item["CODE_PRODUCT"])] = [
                        item["SK"]
                    ]
                else:
                    stat_code["{}@{}".format(id_cate, item["CODE_PRODUCT"])] += 1
                    stat_SK["{}@{}".format(id_cate, item["CODE_PRODUCT"])].append(
                        item["SK"]
                    )
    total_count = 0
    brand_product_save = []
    ...
    print(
        "\t- upper_2_in_product",
        count_product_has_more_than_2_item,
        "total_item",
        total_count,
    )
