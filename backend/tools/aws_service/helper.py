...
class S3Helper(object):
    ...
    def copy_data_to(self, data_finished_dir):
        shutil.copytree(os.path.join(self.root_dir, 'data/raw'), 
               os.path.join(self.root_dir, data_finished_dir), ignore=shutil.ignore_patterns('.aws-sam', 'node_modules'),dirs_exist_ok=True)
    ...
    def is_exist_data(self, page_item):
        dst_dir = os.path.join(self.root_dir,'data/goods' ,page_item['PK'], page_item['SK']) 
    ...
    def write_handled_data(self, new_goods_item):  
        ...
        dst_dir = os.path.join(self.root_dir,'data/pre-goods',today ,new_goods_item['CATEGORY'],new_goods_item['PK'], new_goods_item['SK']) 
    ...
    def upload_to_s3(self):
        ...
        command = ' '.join([
            ...
            "s3://{}-{}-data-bucket/{}".format(self.project_name, self.env_name, os.path.basename(self.config['function_name'])),
