...
const fetchMetaData = (url, callback) => {
    downloadContent(url, (isOk, result, headers) => {
        // console.log('downloadContent ...', url, isOk);
        if (!isOk) {
...
function isGetMetaData(request) {
    ...
    if (['.js', '.css', '.html', '.png', '.jpeg', '.jpg'].includes(parsedPath.ext)) return false;
    ...
    if (metaUrl != '') {
        ...
        fetchMetaData(metaUrl, (isOk, metaTags, metaHeaders) => {
            ...
            if (!isOk) {
                ...
                console.log('contentUrlObj', contentUrlObj)
...
async function main() {
    const eventTest = [
        ...
        // '/home',
        // '/category/54161',
        // '/(FREESHIP)-Smart-Tivi-Samsung-4K-Crystal-UHD-75-inch-UA75AU7000---Chinh-hang-BH-2-nam---Mien-phi-lap-dat-i.893532375612.12281154161'
    ]
    const context = {
        "awsRequestId": "e104699f-a82a-4893-ae18-b843dc5e0119",
        "callbackWaitsForEmptyEventLoop": true,
        "functionName": "pcw-ssr-SSRFunction",
        "functionVersion": "$LATEST",
        "invokedFunctionArn": "arn:aws:lambda:us-east-1:194560059530:function:pcw-ssr-SSRFunction",
        "logGroupName": "/aws/lambda/pcw-ssr-SSRFunction",
        "logStreamName": "2022/09/15/[$LATEST]2c5296a593914329997483eeef3fb250",
        "memoryLimitInMB": "128"
    }

    for (const url of eventTest) {
            ...
            "Records": [
                {
                    "cf": {
                        ...
                        "request": {
                            ...
                            "method": "GET",
                            // "uri": "/android-tivi-casper-43-inch-43fg5200-bao-hanh-24-thang-mien-phi-giao-hang-hn-hcm-i1414163530.html.477830848566.83675954161"
                            "uri": url,
                        }
                    }
                }
