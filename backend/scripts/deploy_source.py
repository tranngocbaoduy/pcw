...
def main(args={}):
    ...
    if not args["skip_build"]:
        ...
        source_bucket_local_dir = root_dir + "/build/.packages/{}".format(
            args["env_name"]
        )
        ...
        package_dirnames = [
            item for item in os.listdir(packages_dir) if item.endswith("-lambda")
        ]
        # package_dirnames = []
        ...
        for dirname in package_dirnames:
            ...
            package_config_file = os.path.abspath(
                os.path.join(packages_dir, dirname, "scripts/config.json")
            )
            ...
            with open(os.path.join(packages_dir, dirname, "scripts/config.json")) as f:
                config = json.load(f)
                AWSHelper.copy_file(
                    os.path.join(packages_dir, dirname, config["function_zip_rpath"]),
                    os.path.join(
                        source_bucket_local_dir,
                        os.path.basename(config["function_zip_rpath"]),
                    ),
                )
        ...
        " ".join(
            [
                ...
                "--template-file={}".format(
                    os.path.join(root_dir, "templates/source-bucket.template")
                ),
                "--stack-name={}-{}-source-bucket".format(
