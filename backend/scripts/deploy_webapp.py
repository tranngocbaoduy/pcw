...
webapp_bucket_local_dir = os.path.join(
    root_dir, "build/webapp-bucket/{}".format(args["env_name"])
)
...
try:
    ...
    cur_stack_outputs = cloudformation_client.describe_stacks(
        StackName=main_stack_name
    )["Stacks"][0]["Outputs"]
    rest_api_base_url = next(
        item["OutputValue"]
        for item in cur_stack_outputs
        if item["OutputKey"] == "RestApiBaseUrl"
    )
except:
    pass
...
version = (
    cur_version
    + "."
    + (datetime.datetime.utcnow().replace(microsecond=0).isoformat() + "Z").replace(
        ":", "_"
    )
)
...
if not args["skip_build"]:
    ...
    AWSHelper.run_command(" ".join(["npm", "run", "build", "--prefix", webapp_dir]))
...
web_acl_arn = ""
try:
    waf_stack_outputs = session.client(
        "cloudformation", region_name="ap-southeast-1"
    ).describe_stacks(StackName=waf_stack_name)["Stacks"][0]["Outputs"]
    web_acl_arn = next(
    ...
    " ".join(
        [
            ...
            "--template-file={}".format(
                os.path.join(root_dir, "templates/webapp-bucket.template")
            ),
            "--stack-name={}-{}-webapp-bucket".format(
