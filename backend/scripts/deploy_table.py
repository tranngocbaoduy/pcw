...
def init(args):
    ...
    cur_version = ''
    try:
        cur_stack_parameters = session.client('cloudformation').describe_stacks(StackName=main_stack_name)['Stacks'][0]['Parameters']
...
def main(args={}):
    ...
    AWSHelper.run_command(' '.join([
        ...
        "--parameter-overrides",
        ' '.join(['{}={}'.format(key, template_parameters_map[key]) for key in template_parameters_map]),
        "--tags",
