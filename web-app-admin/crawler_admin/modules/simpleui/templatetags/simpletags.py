...
def load_dates(context):
    ...
    return '<script type="text/javascript">var searchDates={}</script>'.format(
...
@register.simple_tag
def get_app_info():
    return format_table({"version": get_version()})
            ...
            [
                {
                    "name": m.get("name"),
                    "icon": get_icon(
                        m.get("object_name"), unicode_to_str(m.get("name"))
                    ),
                    "url": m.get("admin_url"),
                ...
                if "models" in i:
                    for k in i.get("models"):
                        k["breadcrumbs"] = [
                            ...
                            {"name": k.get("name"), "icon": k.get("icon")},
                        ]
                else:
                    i["breadcrumbs"] = [{"name": i.get("name"), "icon": i.get("icon")}]
                data.append(i)
        else:
...
def load_message(context):
    ...
    array = (
        [dict(msg=msg.message, tag=msg.tags) for msg in messages] if messages else []
    )
