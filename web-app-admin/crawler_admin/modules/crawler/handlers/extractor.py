...
class ExtractorService(object):
    ...
    def handle_extract_information_from_json(response):
        ...
        base_item = dict() 
        # base_item['id_pcw'] = response['id_pcw']
        base_item['name'] = response['name'].strip()
        base_item['url'] = response['url']
        base_item['domain'] = domain
        base_item['agency'] = agency
        base_item['clean_name'] = ExtractorService.handle_get_clean_name(response['name']).strip() 
        base_item['category'] = response['category_code'] 
        # base_item['category_code_from_title'] = ExtractorService.handle_get_category(response['name'])

        if domain == 'lazada.vn': 
            base_item['slug_id'] = response['itemUrl'].replace('//www.lazada.vn/products', '')
            base_item['brand'] = response['brandName'].strip()
            base_item['image'] = [response['image'].strip()] 
            ...
            base_item['list_price'] = response.get('originalPrice', response.get('price', 0))
            base_item['stock'] = 10 if response['inStock'] else 0
            ...
            base_item['item_rating'] = {
                ...
                "rcount_with_context": int(response.get('review', 0) if response.get('review', 0) else 0),
                "rcount_with_image": 0
            }
            base_item['product_code'] = ExtractorService.handle_get_code(
                response['name'], response['category_code'], base_item['brand'].lower())
            # base_item['content'] = response['short_description'].strip()
            # base_item['voucher_info'] = None
            base_item['description'] = response.get('description', []) 

        if domain == 'tiki.vn': 
            official_shop = [] #list(filter(lambda x: x.get('id', '') == response.get('shopid', None) , list_seller))
            ...
            base_item['slug_id'] = '/{}'.format(response['url_key'])
            base_item['brand'] = response['brand_name'].strip()
            base_item['image'] = [response['thumbnail_url'].strip()] 
            base_item['price'] = response['price'] 
            base_item['list_price'] = response['list_price'] 
            base_item['stock'] = response['stock_item']['qty'] if response['stock_item'] else 0
            base_item['historical_sold'] = response['quantity_sold']['value'] if response['quantity_sold'] else 0
            base_item['liked_count'] = 0
            base_item['item_rating'] = {
        ...
        if domain == 'shopee.vn':  
            official_shop = list(filter(lambda x: x.get('shopid', '') == response.get('shopid', None) , list_seller))
            if len(official_shop) == 1: 
                official_shop = official_shop[0] 
                base_item['shop'] = {
            ...
            base_item['brand'] = response['brand']       
            base_item['image'] = ['https://cf.shopee.vn/file/{}'.format(image) for image in response['images']]
            base_item['stock'] = response['stock']
            base_item['item_rating'] = response['item_rating']
            base_item['historical_sold'] = response['historical_sold']
            base_item['liked_count'] = response['liked_count'] 
            base_item['price'] = int(response['price']) / 100000
            base_item['list_price'] = int(response['price_max']) / 100000 
            base_item['slug_id'] = response['slug_id']
            # base_item['voucher_info'] = response['voucher_info']

            not_allow_brand = ['nobrand','no brand', 'No brand','No Brand', 0,'0', '', 'none']
            if (base_item['brand'] == 0 or base_item['brand'].lower() not in not_allow_brand  or len(base_item['brand']) == 0) and base_item['brand_from_title'] and len(base_item['brand_from_title']) != 0:
                base_item['brand'] = base_item['brand_from_title']
            base_item['product_code'] = ExtractorService.handle_get_code(
                response['name'], response['category_code'],base_item['brand'].lower())
            base_item['description'] = []
 
        base_item['image'] = json.dumps(base_item['image']) 
        return base_item
    ...
    def handle_get_clean_name(text): 
        ...
        text = re.sub('/[^a-z0-9A-Z_ÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚĂĐĨŨƠàáâãèéêìíòóôõùúăđĩũơƯĂẠẢẤẦẨẪẬẮẰẲẴẶẸẺẼỀỀỂưăạảấầẩẫậắằẳẵặẹẻẽềềểỄỆỈỊỌỎỐỒỔỖỘỚỜỞỠỢỤỦỨỪễếệỉịọỏốồổỗộớờởỡợụủứừỬỮỰỲỴÝỶỸửữựỳỵỷỹ]/u', '', text)
        text = re.sub(' +', ' ', text)
        count = 0
        while count < 2:
    ...
    def get_config_filter_of_category(category):
        ...
        if category == 'ELECTRIC_STOVE':
            ...
            list_stop_word = ['galaxy','-','iphone','redmi', 'fullbox', 'pixel', 'z' ,'rog', 'phone', 'gaming', 'reno', 'note', 'xperia']
            ...
            allowed_brand = [
                'apple', 'samsung', 'xiaomi', 'oppo', 'sony', 'vivo', 'asus',
                # 'lenovo', 'huawei',  'google', 'microsoft', 'vsmart', 
                # 'lp', 'realme', 'sony', 'vivo'
            ]
        elif category == 'FRIDGE':
            list_stop_word = [
                'inverter', 'side', 'by', ',', 'mini', 'lan', "multidoor","twin","multi","hai","door",
                'nha', 'dung', 'cao', 'sinh', '90%', 'lanh', 'zin','electric', 'qua', 'motor', 'cho.', 'in', 'cho', 
                'hcm', '13hn', '553/515', 'dc8v',
            ]
            list_keep_word = []
            allowed_brand = [
                'aqua',
               'alaska',  'casper', 'bosch', 'electrolux', 'funiki', 'hitachi',
               'panasonic', 'samsung', 'sanyo', 'sharp', 'toshiba', 
            ...
            list_stop_word = [ 
                "aspire","digital","cooling","swift","travelmate", "plus", "x", "b3", "gaming", "macbook", "mac",'vivobookslate','vivobook','zenbook',
                "flip", "oled", "evo" ,"tuf", "expertbook", "f15", "14x", "15x", "rog", "strix", "dash", "15s", "14s", "essential", "notebook", "core", "340s", 'intel', 'probook', "x360",
                "gram", "modern", 'series', 'ship', '1tr5'
            ]
            is_allowed_2_digits = True
        elif category == 'TELEVISION':
            list_stop_word = [
                "inch", 'led', 'wifi', '-', 'crystal', 'hd', 'uhd', 'smart', 'tv', 'tivi', 'android', 'model', '10.0', '9.0',
                "the", "qled", "neo", "ltv", "full", "inchsmart", "khung", "fhd", "serif", "frame", 'mini',
                "freestyle", 'islim', 'series', 'thanh', 'new', 'udh', 'oled'
            ]
            allowed_brand = [
                'aqua', 'asanzo', 'casper', 'tcl', 'coocaa', 'samsung', 'sharp', 'sony', 'lg', 'panasonic', 'philips', 
            ...
            list_stop_word = []
            is_allowed_2_digits = False 
        return list_stop_word, is_allowed_2_digits, remove_word, list_keep_word, dict_replace_word, allowed_brand
    ...
    def is_word_valid(text, category):
        if category == 'FRIDGE' and\
             (re.match(r"[0-9]+l", text) or\
             re.match(r"[0-9]+%", text) or\
             re.match(r"[0-9]+v", text) or\
             re.match(r"[0-9]+[0-9]+", text) or\
             re.match(r"[0-9]+.[0-9]+cm", text) or\
             re.match(r"[0-9]+.[0-9]+w", text) 
             ): 
            return False
    ...
    def get_last_n_word(list_text, from_index, category, n=3): 
        list_stop_word, is_allowed_2_digits, remove_word, list_keep_word, dict_replace_word, _ = ExtractorService.get_config_filter_of_category(category)
        last_n_word = []
        from_index += 1
        count = 0 
        check = False
        for i, text in enumerate(list_text):
            ...
            if i == from_index:
                ...
                if text not in list_stop_word:  
                    if not ExtractorService.is_word_valid(text, category): continue
    ...
    def handle_get_code(text, category, brand): 
        _, _, _, _, dict_replace_word, allowed_brand = ExtractorService.get_config_filter_of_category(category)
        if brand in allowed_brand or len(allowed_brand) == 0:
            text = ExtractorService.replace_word_consistent(text.lower(), dict_replace_word)
            ...
            if brand in split_str_name:
                ...
                last_n_word = ExtractorService.get_last_n_word(split_str_name, index_of_brand, category, n=1) 
                if len(last_n_word) == 0: return "NONE"
    ...
    def handle_get_clean_code(text):
        ...
        words1 = set(text)
        return ' '.join(list(filter(lambda x: len(x) > 4, list(words1.difference(common_words)))))
    ...
    def handle_compare_list_price_and_price(item):
        ...
        elif item['price'] and item['list_price'] and item['price'] > item['list_price']:
        ...
        for selector in selector_items:
            try:
                ...
                for img_element in image_element_items:
                    ...
                    if src:
                        if 'https:' not in src: src = 'https:{}'.format(src)
    ...
    def get_currency_from_text(text): 
        ...
        if price.amount_float and price.amount_float > float(999): return price.amount_float
        return None
    ...
    def handle_get_category(text):
        ...
        with open(os.path.join(root_dir, 'handlers/config_file/category_code_priority.json')) as f:
        ...
        for _dir in product_type_file_dirs:
            if '.txt' not in _dir:  
                ...
                words2 = [i.replace('\n','').lower() for i in open(os.path.join(file_dir, _dir)).readlines()]
                is_exist = False 
                for word in words2:
                    if word in text:
                        is_exist = True
                        break
                if is_exist: product_types.append(product_type)  
    ...
    def calculate_score(base_item):
        ...
        # score for rating 
        if base_item['item_rating'] and str(base_item['item_rating']['rating_star']) != '0':
            base_score += 2
        ...
        if base_item['shop'] != None and base_item['shop'].get('store_level', '') == 'TRUSTED_STORE':
            base_score += 6
        elif base_item['shop'] != None and base_item['shop'].get('store_level', '') == 'OFFICAL_STORE':
            if base_item['category_code'] == 'FRIDGE': base_score += 5
            if base_item['category_code'] == 'TELEVISION': base_score += 5
            if base_item['category_code'] == 'PHONE': base_score += 4
        elif base_item['shop'] != None:
            if base_item['category_code'] == 'FRIDGE': base_score += 4
            if base_item['category_code'] == 'TELEVISION': base_score += 4
            if base_item['category_code'] == 'PHONE': base_score += 3
        else: 
            base_score += 2
    ...
    @staticmethod
    def make_new_dir(init_dir, id_pcw, init_category_code, update_category_code, brand_name, allowed_category,from_dir='data_crawler',):
        _dir_save_data = ''
        ...
        if update_category_code in allowed_category:
            ...
            _dir_save_data = init_dir.replace(from_dir,'data').replace(init_category_code, update_category_code)
          
            # add brand name to path
            _dir_save_data = _dir_save_data.replace(update_category_code, '{}/{}'.format(update_category_code, brand_name))
    ...
    def get_config_item(domain, category):
        config = ExtractorService.read_data_json(os.path.join(root_dir,'product_crawler/spiders/setting/{}/{}.json'.format(domain, category)))
