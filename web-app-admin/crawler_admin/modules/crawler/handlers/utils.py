...
class SettingService(object):
    ...
    def get_all_brand_dirs():
        settings_file_dir = os.path.join(root_dir, 'product_crawler/spiders/setting/brands')
    ...
    def get_all_setting_brand_items():
        ...
        for setting_brand_dir in setting_brand_dirs:
            ...
            setting_brands[setting_brand_name] = SettingService.get_setting_brand(setting_brand_dir)
...
class CrawlingHelper(object):
    ...
    def get_name_by_time(cur_version=''):
        if cur_version != '':
            cur_version += '.' 
        return cur_version + (datetime.datetime.utcnow().replace(microsecond=0).isoformat() + 'Z').replace(':', '_')

    @staticmethod
    def get_now():
        return (datetime.datetime.utcnow().replace(microsecond=0).isoformat() + 'Z').replace(':', '_')
