...
from modules.crawler.models import News, Category
from modules.crawler.serializers import NewsSerializer, CategorySerializer
...
class RecommendNewsList(APIView):
    ...
    def get(self, request, pk, format=None):
        ...
        allNews = News.objects.filter(
            published_at__range=[beforeNews, nextNews]
        ...
        for new in allNews:
            id_idx[new.id] = i
            idx_id[i] = new.id
            document_embeddings[i] = model.infer_vector(
                ViTokenizer.tokenize(new.content).split()
            )
            i = i + 1
        ...
        if news not in allNews:
            ...
            idx_id[i] = news.id
            document_embeddings[i] = model.infer_vector(
                ViTokenizer.tokenize(news.content).split()
            )
...
class CategoryNewsList(generics.ListAPIView):
    ...
    def get_queryset(self):
        categories = (
            Category.objects.get(pk=self.kwargs.get("pk"))
