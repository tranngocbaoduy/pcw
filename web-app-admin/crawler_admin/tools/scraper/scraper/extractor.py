...
class ExtractorService(object):
    ...
    def handle_extract_information_from_json(response, domain, list_seller):
        ...
        if len(response['base_encoded_url']) > 250: print(response)
        ...
        product_item['clean_name'] = ExtractorService.handle_get_clean_name(response['name']).strip() 
        product_item['category_code'] = response['category_code']
        product_item['main_category_code'] = response.get('main_category_code', 'NONE')
        product_item['category_code_from_title'] = ExtractorService.handle_get_category(response['name'])
        product_item['brand_from_title'] = SettingService.get_brand_from_name(response['name'], setting_brand_items)

        if domain == 'lazada.vn': 
            product_item['slug_id'] = response['itemUrl'].replace('//www.lazada.vn/products', '')
            ...
            product_item['list_price'] = response.get('originalPrice', response.get('price', 0))
            ...
            product_item['product_code'] = ExtractorService.handle_get_code(
                response['name'], response['category_code'], product_item['brand'].lower())
        ...
        if domain == 'tiki.vn': 
            ...
            product_item['stock'] = response['stock_item']['qty'] if response['stock_item'] else 0
            product_item['historical_sold'] = response['quantity_sold']['value'] if response['quantity_sold'] else 0
            product_item['liked_count'] = 0
            product_item['shop_location'] = official_shop.get('seller_address', '').split(',')[-1].strip() if official_shop and official_shop.get('seller_address', '') else 'Tiki Trading'
            product_item['item_rating'] = {
        ...
        if domain == 'shopee.vn':  
            ...
            product_item['image'] = ['https://cf.shopee.vn/file/{}'.format(image) for image in response['images']]
            ...
            if (product_item['brand'] == 0 or product_item['brand'].lower() not in not_allow_brand  or len(product_item['brand']) == 0) and product_item['brand_from_title'] and len(product_item['brand_from_title']) != 0:
    ...
    def handle_get_category(text):
        ...
        with open(os.path.join(root_dir, 'product_crawler/spiders/setting/category_code_priority.json')) as f:
    ...
    def calculate_score(base_item):
        ...
        if base_item['shop_item'] != None and base_item['shop_item'].get('store_level', '') == 'TRUSTED_STORE':
            base_score += 6
        elif base_item['shop_item'] != None and base_item['shop_item'].get('store_level', '') == 'OFFICAL_STORE':
